...
spp.showEquation(beta, dvTotSimplified)
spp.printMarkdown(r'The problem explicitly states that we should simplify this into the most compact form.  I tried various techniques in sympy to do that, but this may not be exactly what the author is looking for.')
...
t = np.arange(1, 40.0, 0.01)
    # here is one of the most powerful features of sympy!  Eventually, you want to 
...
dvHoh = sy.lambdify(alpha, dvTotSimplified)(t)
dvPar = sy.lambdify(alpha, dvParSubs)(t)

fig, ax = plt.subplots()
ax.plot(t, dvHoh, label="Hohmann")
...
spp.printMarkdown(r'Here we will use a numerical root-finder to find when the two transfer techniques equal each other.  Attempts to solve it symbolically failed.')
...
alphaEqLhs = sy.lambdify(alpha, dvTotSimplified)
alphaEqRhs = sy.lambdify(alpha, dvParSubs)
def EqToSolve(alp) :
...
#%%
spp.printMarkdown(r'### Acknowledgments {-}')

spp.printMarkdown("Many thanks for the Citation Style Language website for making citations so easy and simple @CslDefinition.  Also thanks to the AIAA for publishing their csl file @AiaaCslDef.  Also, this sample of the Sympy Paper Printer is made with the Sympy Paper Printer and is in the public domain as specified on its github page @SppRepo. Please forgive, but also point out, any mistakes or problems or potential improvements.  Especially with how citations are handled.")
